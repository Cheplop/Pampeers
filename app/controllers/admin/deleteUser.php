...
// Get the user ID to delete from the form, convert to integer
...
// Prepare a query to update the user: set isActive to 0 and add deletion time, but only if not admin
$stmt = $conn->prepare("
    UPDATE users
...
if ($stmt->execute()) {
    $stmt->close();
    header('Location: /Pampeers/public/admin/dashboard.php?success=user_deleted');
    exit();
}

// If update failed, close statement and redirect with error
$stmt->close();
header('Location: /Pampeers/public/admin/dashboard.php?error=delete_failed');
exit();
?>