...
// Joined tables using the exact correct columns from your users and sitters table
$query = "SELECT s.*, 
                 CONCAT(u.firstName, ' ', u.lastName) as name, 
                 u.profilePic as img, 
                 u.cityMunicipality as city,
                ...
                <div class="col">
                    <div class="small-card bg-white p-3 shadow-sm rounded-4">
                        <div class="card-img-container mb-2">
                            <img src="/Pampeers/app/uploads/profiles/<?= htmlspecialchars(!empty($peer['img']) ? $peer['img'] : 'default.jpg'); ?>" 
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="fw-bold text-primary">₱<?= htmlspecialchars($peer['rate'] ?? '0') ?>/hr</span>
                            <a href="bookSitter.php?sitterID=<?= htmlspecialchars($peer['sitterID']) ?>" class="btn btn-sm btn-dark rounded-pill px-3">Book</a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
